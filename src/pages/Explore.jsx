...
const Explore = () => {
  ...
  const showSearchedMovies = () => {
    return (
      <>
        <h2 className="text-center text-gray-400 text-lg mb-3">
          Your Search Results
        </h2>
        <ul className="list-none  w-full h-full pb-20 align-center flex items-center flex-wrap overflow-auto  scrollbar-hide gap-3 md:gap-7">
  ...
  const initialTime = () => {
    return (
      <h1 className="text-center text-gray-400 text-lg m-6">
        Search above for movies or use filters
      </h1>
    );
  };
  const failureView = () => {
  ...
  return (
    <>
      <Header />
      <div
        className="show-case flex justify-end w-full z-20 h-full fixed duration-500"
        style={{
      ...
      <div className="bg-gradient-to-b from-[#121212] to-[#1e1e1e]  max-w-[100vw] min-h-[92vh] p-5 box-border">
        <div className="w-[90%] mx-auto flex items-center gap-4">
          <input
            ...
            className="border-amber-50 text-amber-100 border-2 h-10 rounded-2xl w-[80%] outline-none p-4 "
          />
        ...
        <div className=" border-amber-100 h-[75vh] w-[100%]">{showView()}</div>
      </div>
    </>
  );
};
